...
async def submit_stem_separation(
    ...
    if len(body.stems) > settings.MAX_STEMS_PER_REQUEST:
        raise HTTPException(
            status_code=400,
            detail=f"Too many stems requested. Maximum {settings.MAX_STEMS_PER_REQUEST} stems per request. You requested {len(body.stems)} stems."
        )
    ...
    existing_stems = await db.execute(
        select(Stem).where(
            ...
            Stem.stem_type.in_([StemType(s) for s in body.stems])
        )
    )
